...
export default {
  ...
  computed: {
    computedRefreshDates(){
      return this.refreshDates || this.filters.refreshDates.selected
    },
    computedCategories(){
      return this.categories || this.filters.categories.selected
    },
    computedCustomers(){
      return this.customers || this.filters.customers.selected
    },
    computedValueOrQuantity(){
      return this.valueOrQuantity || this.filters.valueOrQuantity
    }
  },
  async created() {
    ...
    this.selectFilterUpdated("customers", [ALL_OPTION])
...
<template>
  <div class="tw-flex tw-gap-x-3 tw-w-full tw-bg-white tw-px-3">
    <div class="tw-pt-3 tw-min-w-[14%] tw--mb-3">
      <div>Month & Year</div>
      <VueDatePicker v-model="computedRefreshDates" month-picker @update:model-value="refreshDateUpdated">
    ...
    <div class="tw-pt-3 tw-min-w-[14%] tw--mb-3">
      ...
      <v-select
        ...
        :model-value="computedCategories"
    ...
    <div class="tw-flex tw-gap-1.5 tw-pl-3 tw--mb-3">
      <p :class="`tw-pt-6 ${computedValueOrQuantity === BY_VALUE ? 'tw-font-medium' : ''}`">Value {{currency}}</p>
      <div class="tw-flex tw-pt-2" style="color: #7823DC;">
        <v-switch :model="filters.valueOrQuantity" inset @click="filtersUpdated"/>
      </div>
      <p :class="`tw-pt-6 ${computedValueOrQuantity === BY_QUANTITY ? 'tw-font-medium' : ''}`">Volume</p>
    </div>
  </div>
</template>
<style>
